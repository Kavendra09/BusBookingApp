...
const Bookings = () => {
  ...
  const filteredBookings = selectedTab === 'All' ? ticket : ticket.filter((ticket: any) => ticket.status)
      ...
      <FlatList 
        ListHeaderComponent={<>
          ...
          <View className='flex-row mb-4 '>
            {tabs?.map(tab => (
              <TouchableOpacity key={tab} onPress={() => setSelectedTab(tab)} className={`px-4 py-2 rounded-lg mx-2 ${selectedTab === tab ? 'bg-red-500' : 'bg-gray-300'}`}>
